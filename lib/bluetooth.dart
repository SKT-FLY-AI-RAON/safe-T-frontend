...
class BluetoothHandler {
  ...
  Future<bool> startBluetoothProcess() async {
    await checkPermissions();
    for (var device in pairedDevices) {
      // 이름에 "OBD"가 포함된 장치를 찾습니다.
      if (device.name != null && device.name!.toLowerCase().contains('obdii')) {
        try {
          await connectToDevice(device);
          if (selectedDevice != null)
            return true; // 성공적으로 연결된 경우 true 반환
        } catch (e) {
          return false; // 연결 실패 시 false 반환
        }
      }
    }
    return false; // OBD-II 장치를 찾지 못한 경우 false 반환
    // connectToOBDDevice();
    // await Future.delayed(Duration(seconds: 10));
    // if (selectedDevice == null) {
    //   print("No device connected.");
  ...
  // OBD 장치로 자동 연결하는 함수
  // Future<bool> connectToOBDDevice() async {
  //   for (var device in pairedDevices) {
  //     // 이름에 "OBD"가 포함된 장치를 찾습니다.
  //     if (device.name != null && device.name!.toLowerCase().contains('obdii')) {
  //       try {
  ...
  void parseRPMResponse(String response) {
    ...
    if (parts.length >= 3) {
      ...
      if (parts[0] == "41") {
        switch (parts[1]) {
          case "0C": // 엔진 RPM
            rpm = (int.parse(parts[2], radix: 16) * 256 +
                int.parse(parts[3], radix: 16)) ~/
                4;
            print('Engine RPM: $rpm');
            break;
          case "0D": // 차량 속도
            speed = int.parse(parts[2], radix: 16);
            print('Vehicle Speed: $speed km/h');
            break;
          case "04": // 엔진 부하
            load = int.parse(parts[2], radix: 16) * 100 ~/ 255;
            print('Engine Load: $load%');
            break;
          case "11": // 스로틀 위치
            throttle = int.parse(parts[2], radix: 16) * 100 ~/ 255;
            print('Throttle Position: $throttle%');
            break;
          case "49": // 가속 페달 위치
            pedal = int.parse(parts[2], radix: 16) * 100 ~/ 255;
            print('Accelerator Pedal Position: $pedal%');
            break;
          case "03": // 연료 시스템 상태
            fuelSystemStatus = int.parse(parts[2], radix: 16);
            print('Fuel System Status: $fuelSystemStatus');
            break;
          default:
            print("Unknown PID");
